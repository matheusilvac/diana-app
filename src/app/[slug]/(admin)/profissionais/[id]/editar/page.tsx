...
import { getProfessionalById, updateProfessional } from "@/services/profissionais"
...
export default function EditarProfissionalPage() {
  ...
  const { data: professional, isLoading } = useQuery({
    queryKey: ["professionals", slug, id],
    queryFn: () => getProfessionalById(slug, id),
  ...
  React.useEffect(() => {
    ...
    form.reset({
      ...
      specialties: professional.specialties as any,
      days: professional.schedule.days as any,
      start: professional.schedule.start,
      end: professional.schedule.end,
    })
  }, [form, professional])

  const mutation = useMutation({
    mutationFn: async (values: FormValues) => {
      return updateProfessional(slug, id, {
        name: values.name,
        avatar: values.avatar || `https://i.pravatar.cc/150?u=${encodeURIComponent(values.name)}`,
        specialties: values.specialties,
        schedule: { days: values.days, start: values.start, end: values.end },
      })
    },
    onSuccess: async (prof) => {
      await queryClient.invalidateQueries({ queryKey: ["professionals", slug] })
      await queryClient.invalidateQueries({ queryKey: ["professionals", slug, id] })
      toast.success("Profissional atualizado", { description: prof.name })
    ...
    return (
      <FadeIn>
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">ID inválido.</p>
          <Button asChild variant="outline" className="border-border">
            <Link href={`/${slug}/profissionais`}>Voltar</Link>
          </Button>
        </div>
      </FadeIn>
    )
  ...
  if (!isLoading && professional === null) {
    return (
      <FadeIn>
        <div className="space-y-2">
          <h1 className="text-2xl font-bold text-foreground">Editar profissional</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Ajuste os dados do colaborador. O envio ao backend será conectado depois.
                </p>
              </div>
            </div>
          </div>
